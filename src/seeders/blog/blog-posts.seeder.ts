...
export class BlogPostsSeeder {
    ...
    if (!posts || posts.length === 0) {
      try {
        for (const item of blogPostsMock) {
          ...
          const existCategory = await this.blogCategoriesRepository.findOneBy({
            name: category,
          });
          const existTemplate = await this.blogTemplatesRepository.findOneBy({
            name: template,
          });
  ...
  private async chechDbData(title: string) {
    const post: BlogPost | null = await this.blogPostsRepository.findOneBy({
      title,
    });
    return !!post;
  }
