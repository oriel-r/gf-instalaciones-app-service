...
export class OrdersSeeder {
  async seed(): Promise<void> {
    ...
    await appDataSource.transaction(async manager => {
      ...
      if (!addresses.length || !clientsPool.length || !coordsPool.length || !installers.length) {
      ...
      savedOrders.forEach(order => {
        ...
        if (rnd < 0.33) { // done
          ...
          order.progress = 100;
        } else if (rnd < 0.66) { // half
          completedCnt = count > 1 ? Math.floor(Math.random() * (count - 1)) + 1 : 0;
        ...
        mocks.forEach(mock => {
          ...
          installs.push(
            manager.create(Installation, {
              ...
              coordinator: [...coordsPool].sort(() => 0.5 - Math.random()).slice(0, 2),
              installers : [...installers ].sort(() => 0.5 - Math.random()).slice(0, 3),
              startedAt           : mock.status !== InstallationStatus.PENDING ? startDt : undefined,
              submittedForReviewAt: mock.status === InstallationStatus.TO_REVIEW
                                    ? new Date(+startDt + 86_400_000) : undefined,
              endDate             : mock.status === InstallationStatus.FINISHED
                                    ? new Date(+startDt + 172_800_000) : undefined,
              notes: mock.notes,
              images: (mock.status === InstallationStatus.TO_REVIEW
