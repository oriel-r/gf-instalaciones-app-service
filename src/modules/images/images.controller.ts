...
export class ImagesController {
    ...
    @UsePipes(new FilePipe(1000, 5000000, ['image/png', 'image/jpeg', 'application/pdf']))
    async uploadFile(@UploadedFile() file: Express.Multer.File) {
        const fileUrl = await this.fileUploadService.uploadFile(file);
        return await this.imageService.saveFile({url: fileUrl, mimetype: file.mimetype});
    }

    @Post('upload/images')
    @UseInterceptors(FilesInterceptor('files', 10))
    @UsePipes(new FilesPipe(1000, 10000000, ['image/png', 'image/jpeg', 'application/pdf']))
