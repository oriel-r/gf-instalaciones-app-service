...
export class ProvinceService {
  ...
  async findAll() {
    ...
    if(!provinces || provinces.length === 0) throw new NotFoundException('No se encontraron provincias')
      return provinces
  }
  ...
  async update(id: string, data: UpdateProvinceDto) {
    ...
    if(!existProvince) throw new NotFoundException('No se encontro la provincia, id invalido')
