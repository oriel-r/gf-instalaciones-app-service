...
export class BlogPostTamplatesService {
  constructor(
    private readonly blogPostTemplatesRepository: BlogPostTemplatesRepository,
  ) {}

  async createTemplate(
    data: CreateBlogPostTemplate,
  ): Promise<BlogPostTemplate | HttpException> {
    const exist = await this.blogPostTemplatesRepository.getByName(data.name);
    if (exist)
      throw new ConflictException(
        'Este plantilla ya existe o tiene un nombre repetido',
      );
  ...
  async getAllTemplates(): Promise<BlogPostTemplate[] | HttpException> {
    ...
    if (!templates || templates.length === 0)
      throw new NotFoundException('No se encontraron plantillas');
    return templates;
  }
  ...
  async softDeleteTemplate(
    id: string,
  ): Promise<DeleteResponse | HttpException> {
    const template = await this.blogPostTemplatesRepository.getById(id);
    if (!template) throw new NotFoundException('No se encontro la plantilla');
