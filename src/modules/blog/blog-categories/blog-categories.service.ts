...
export class BlogCategoriesService {
    constructor(private readonly blogCategoriesRepository: BlogCategoriesRepository) {}
    ...
    async create(data:CreateCategoryDto): Promise<BlogCategory | null> {
        ...
        if(exist) throw new ConflictException('Esta categoria ya existe o tiene un nombre ya utilizado')
    ...
    async get(): Promise<BlogCategory[] | void[]> {
        ...
        if(!categories || categories.length < 0) throw new NotFoundException('No se encontraron categorias')
        return categories
    }
