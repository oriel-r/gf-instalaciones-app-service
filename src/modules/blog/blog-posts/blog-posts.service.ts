...
export class BlogPostsService {
  ...
  async get(category?: string): Promise<BlogPost[] | void[]> {
    ...
    if (category) {
      const existCategory =
        await this.blogCategoriesService.getByName(category);
      if (!existCategory) {
        throw new NotFoundException(
          `No existe la categoría con nombre "${category}"`,
        );
      }
  ...
  async softDelete(id: string): Promise<DeleteResponse | undefined> {
    ...
    if (!existPost)
      throw new NotFoundException('Post no encontrado o ID invalido');
