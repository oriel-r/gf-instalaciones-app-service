...
export class AddressService {
  ...
  async create(data: CreateAddressDto) {
    ...
    if (!existProvince)
      throw new BadRequestException('Provincia no encontrada o invalida');
    const existCity = await this.citiesService.findOrCreate(
      city,
      existProvince,
    );
  ...
  async findAll() {
    ...
    if (!Addresses.length)
      throw new NotFoundException('No se encontraron direcciónes');
  ...
  async update(id: string, data: DeepPartial<CreateAddressDto> | undefined ) {
    ...
    if (city || province) {
      ...
      updatedCity = await this.citiesService.findOrCreate(
        cityNameToUse,
        provinceToUse,
      );
    }
    ...
    if (!result.affected)
      throw new InternalServerErrorException(
        'Error desconcodio al itnetnar actulizar la dirección',
      );
  ...
  async remove(id: string) {
    ...
    if (!Address)
      throw new NotFoundException(
        'No se enconro  la dirección o el id es invalido',
      );
